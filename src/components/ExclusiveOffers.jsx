...
export default function ExclusiveOffers() {
  return (
    <div className='flex flex-col item-center px-6 lg:px-24 xl:px-32'>
      <div className=' flex flex-col md:flex-row items-center justify-between w-full'>
        <Title align='left' title='Exclusive Offers' subTitle='Lorem ipsum, dolor sit amet consectetur adipisicing elit. Iure, quibusdam veritatis iste quidem debitis suscipit facere alias illum distinctio perspiciatis.'/>
      ...
      {exclusiveOffers.map((item)=>(
        ...
        <p className='text-xs text-white/70 mt-3'>Expores {item.expiryDate}</p>
        </div>
        <button className='flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>
            View Offers<img className='invert group-hover:translate-x-1 transition-all' src={assets.arrowIcon} alt='arrow-icon'/>
        </button>
        </div>
      ))}
      </div>
