...
export default function HotelCard({room,index}) {
  ...
  return (
    <Link to={'/rooms/'+ room._id} onClick={()=>scrollTo(0.0)} key={room._id} style={{ transform: `perspective(1000px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)` }}
      onMouseMove={handleMove}
            onMouseLeave={() => setTilt({ x: 0, y: 0 })}
            className='relative max-w-70 w-full rounded-xl overflow-hidden
            bg-white text-gray-500/90 shadow-[0px_4px_4px-rgba(0,0,0,0.05)] pb-15'
            >
    <img className='relative max-w-70 w-full rounded-xl overflow-hidden bg-white text-gray-500/90 shadow-[0px_4px_4px_rgb(0,0,0,0.05)]' src={room.images[0]} alt=''/>
    ...
    <div className='p-4 pt-5'>
        <div className='flex items-center justify-between'>
            <p className='font-playfair txet-xl font-medium text-gray-800'>{room.hotel.name}</p>
